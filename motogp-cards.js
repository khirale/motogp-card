...
class MotoGPEventCard extends HTMLElement {
  ...
  _eid() {
    ...
    return {
      event:     e.event      || 'sensor.motogp_next_event',
      sessions:  e.sessions   || 'sensor.motogp_sessions',
      race_start:e.race_start || 'sensor.motogp_next_race_start',
    };
  }
  ...
  _render() {
    ...
    const ids = this._eid();
    const evtState  = hass.states[ids.event];
    const sesState  = hass.states[ids.sessions];
    const raceState = hass.states[ids.race_start];
...
class MotoGPTeamsCard extends HTMLElement {
  ...
  _eid() {
    const e = (this._config && this._config.entities) || {};
    return { teams: e.teams || 'sensor.motogp_classement_equipes' };
  }
  set hass(hass) { this._hass = hass; this._render(); }
  ...
  _render() {
    const t      = getT(this._lang());
    const sensor = this._hass.states[this._eid().teams];
    if (!sensor) {
      this.shadowRoot.innerHTML = `<style>${BASE_CSS}</style><ha-card class="card"><div class="empty">${t.sensor_missing}</div></ha-card>`;
      return;
    }
...
class MotoGPLiveCard extends HTMLElement {
  ...
  _eid() {
    const e = (this._config && this._config.entities) || {};
    return {
      live:       e.live       || 'sensor.motogp_live_timing',
      race_start: e.race_start || 'sensor.motogp_next_race_start',
    };
  }
  set hass(hass) { this._hass = hass; this._render(); }
  ...
  _render() {
    ...
    const hass      = this._hass;
    const ids       = this._eid();
    const sensor    = hass.states[ids.live];
    const raceState = hass.states[ids.race_start];
    ...
    const FINISHED_STATUSES = ['finished', 'ended', 'complete', 'completed'];
    const isFinished = FINISHED_STATUSES.includes(status.toLowerCase());
    const showClassification = active || (isFinished && classification.length > 0);
